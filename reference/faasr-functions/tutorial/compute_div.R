compute_div <- function(folder, input1, input2, output) {

  # Download two input files from bucket, generate a division of their contents, and write back to bucket
  ...
  # Print a log message
  # 
  log_msg <- paste0('Function compute_div finished; output written to ', folder, '/', output, ' in default S3 bucket')
  faasr_log(log_msg)
}	